...
encrypt ()
{
    echo "Type 1 for AES256 encryption or 2 for AES128 encryption"
    read input
    if [[ "${input}" -eq 1 ]]; then
    ...
    # Append IV to the end of the fill
   ...
   # Generate the Initialization Vector (IV).
    ...
    # Encrypt (creates a new file)
    openssl enc -aes-128-cbc -in "${in_file}" -out "${out_file}" -iv "${iv}" -K "${enc_key}"
    validate_exit_status $? "encrypting-file" "${out_file}"
...
decrypt ()
{
   ...
   echo "Type 1 for AES256 decryption and 2 for AES128 decryption"
   read input
    if [[ "${input}" -eq 1 ]]; then
    ...
    # Decrypt (creates a new file)
    openssl enc -aes-128-cbc -d -in "${tmp_file}" -out "${out_file}" -iv "${iv}" -K "${enc_key}"
    validate_exit_status $? "decrypting-file" "${tmp_file}"
